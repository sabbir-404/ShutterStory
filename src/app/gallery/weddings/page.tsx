...
export default async function WeddingsIndex() {
  ...
  if (error) {
    return <section className="container"><p className="muted">Failed to load albums.</p></section>;
  }
          ...
          {(albums ?? []).map((a: Album) => (
            <Link key={a.slug} href={`/gallery/weddings/${a.slug}`} className="event-card">
              <div className="event-card-media">
                <Image
                  src={
                    a.cover_thumb_url && a.cover_thumb_url.startsWith("http")
                      ? a.cover_thumb_url
                      : (a.cover_thumb_url || "/assets/hero1.jpg")
                  }
                  alt={a.title}
                  fill
